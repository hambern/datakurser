...
1. Alla, även personen som har skapat `shared_bare.git`, ska nu hämta ner det tomma repot.
2. Skapa en katalog som heter `temp` och flytta er dit. När ni står där ska ni clona repot: `git clone ~username/shared_bare.git`. "username" är användarnamnet till den som skapade repositoryt.
3. Låt nu alla i tur och ordning genomgå följande steg:
...
4. Skapa en egen fil med ditt användarnamn och fyll den med ett innehåll. Lägg till den och gitta in den till repositoryt. Glöm inte att "pulla" repositoryt innan du "pushar"!
5. Gör förändringar i någon annans fil, "committa", "pulla" och "pusha" dina förändringar. Om du blir varnad för conflikter, lös dem med hjälp av kommandot `git merge`
6. Skapa en fil som heter `chat` (om den inte redan finns) och försök föra ett samtal i den med dina kamrater
