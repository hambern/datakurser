...
1. Skapa gärna en egen tabell för valkretsarna `rik_states` och använd `state_id` i `rik_members` för att länka rätt ledamot till rätt valkrets. Det är en bättre databaslösning som bättre följer rekommendationerna på [denna sida](https://www.w3schools.in/DBMS/database-normalization/). Gör sedan en `JOIN` i din sql-fråga för att sätta ihop de två tabellerna i ditt svar.
