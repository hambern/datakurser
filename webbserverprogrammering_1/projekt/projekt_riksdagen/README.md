...
3. Utgå ifrån `index.php` och följ instruktionerna där. När man lägger till `?do=update` ska du tömma din lokala tabell `TRUNCATE`, hämta ut den senaste informationen från riksdagen, lagra informationen i din databastabell `INSERT` och visa informationen på hemsidan `SELECT`.
